...
void CmpInstruction::genMachineCode(AsmBuilder* builder)
{
    ...
    if (opcode >= CmpInstruction::L && opcode <= CmpInstruction::GE) {
        ...
        // std::cout<<dst->getReg()<<std::endl;
        // std::cout<<src1->getReg()<<std::endl;
...
void BinaryInstruction::genMachineCode(AsmBuilder* builder)
{
    ...
    * As to other instructions, such as MUL, CMP, you need to deal with this situation, too.*/
    ...
    if(src1->isImm()&&src2->isImm()){
        //常量折叠
        auto result_IMM = genMachineImm(src1->getVal()+src2->getVal());
        switch (opcode)
        {
        case ADD:
            result_IMM = genMachineImm(src1->getVal()+src2->getVal());
            break;
        case SUB:
            result_IMM = genMachineImm(src1->getVal()-src2->getVal());
            break;
